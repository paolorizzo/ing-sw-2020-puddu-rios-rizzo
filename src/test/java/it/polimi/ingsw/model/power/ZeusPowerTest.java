...
public class ZeusPowerTest extends PowerTest {
                ...
                for(int i=0;i<5;i++){
                    for(int j=0;j<5;j++){
                        ...
                        if(space.hasWorkerOnIt() && space.getWorkerOnIt().toString().equals(workerID))
                            assert(board.getSpaces()[targetX][targetY].getAdjacentSpaces().contains(space) || board.getSpaces()[targetX][targetY] == space);
                    }
                }
            } else {
                assert(false);
